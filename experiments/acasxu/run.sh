...
declare -a dimension_methods=("singular_bound_lower" "random")
for i in "${dimension_methods[@]}"
do
  for TIMEOUT in 60
  do
    OUT_DIR="$HERE/../output/$TIMESTAMP-$i/acasxu/safety/$TIMEOUT"
    mkdir -p "$OUT_DIR"
    for net in "4_3" "4_9" "5_8"
    do
      python -u "$HERE/safety.py" \
      --network "$net" --property 2 --timeout "$TIMEOUT" --decision-method "$i"\
      "$@" \
      | tee "$OUT_DIR/property2_$net.log"
    done
    python -u "$HERE/safety.py" \
    --network "1_9" --property 7 --timeout "$TIMEOUT" --decision-method "$i"\
    "$@" \
    | tee "$OUT_DIR/property7_1_9.log"
    python -u "$HERE/safety.py" \
    --network "2_9" --property 8 --timeout "$TIMEOUT" --decision-method "$i"\
    "$@" \
  ...
  do
    ...
    do
      ...
      do
        python -u "$HERE/safety.py" \
        --network "${i1}_${i2}" --property 2 --timeout "$TIMEOUT" --decision-method "$i"\
        "$@" \
    ...
    do
      python -u "$HERE/safety.py" \
      --network "4_${i2}" --property 2 --timeout "$TIMEOUT" --decision-method "$i"\
      "$@" \
    ...
    do
      python -u "$HERE/safety.py" \
      --network "5_${i2}" --property 2 --timeout "$TIMEOUT" --decision-method "$i"\
      "$@" \
